...
public abstract class BaseBossVisua_mk2 : MonoBehaviour
{
    [Header(" === Visual === ")]
    ...
    [Header(" === Telegraph Eye Flash")]
    [Tooltip("공격 전조 눈빛 조명")] public Light2D eyeLight;
    [Tooltip("눈빛 최대 밝기")]     public float flashIntensity;
    [Tooltip("눈빛 지속 시간")]     public float flashDuration;
    ...
    public virtual void Init(BaseBoss _boss)
    {
        ...
        if (eyeLight != null) eyeLight.intensity = 0;
    ...
    #region 공격 전조 증상 연출
    /// <summary> 공격 전 눈빛 연출 </summary>
    public virtual void PlayEyeFlash()
    {
        if (eyeLight != null)
            StartCoroutine(Co_EyeFlash());
    }

    protected virtual IEnumerator Co_EyeFlash()
    {
        eyeLight.intensity = flashIntensity;
