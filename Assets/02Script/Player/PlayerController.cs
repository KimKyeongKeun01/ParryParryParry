...
public class PlayerController : MonoBehaviour, IPlatformPassenger2D, IBounceable
{
    ...
    private Vector2 currentSlamDir;
    public Vector2 CurrentSlamDir => currentSlamDir;
    private bool slamInterrupted;
    ...
    [SerializeField] private float _slamStartTimeScale = 0.6f;
    [SerializeField] private float _slamStartTimeSlowDuration = 0.08f;
    ...
    private readonly Collider2D[] slamTargetResults = new Collider2D[16];
    ...
    private void ApplySlamStartTimeSlow()
    {
        if (_slamStartTimeSlowDuration <= 0f)
            return;

        if (_slamStartTimeScale >= 1f)
            return;

        ResolveTimeManager();
        if (_timeManager == null)
        {
            Debug.LogWarning("[PlayerController] TimeManager reference is missing.");
            return;
        }

        _timeManager.SetActionTime(_slamStartTimeScale, _slamStartTimeSlowDuration);
    }
    ...
    private void BeginSlamState()
    {
        player.SetSlamState(true);
        player.SetPlayingState(false);
        hasUsedAirSlam = true;
        StopJumpProfile();
        isJumpHeld = false;
    ...
    private void EndSlamStateOnly()
    {
        player.SetSlamState(false);
        rb.mass = defaultMass;
    }

    private void BeginGuardState()
    {
    ...
    private void EndGuardState()
    {
        player.ClearGuardStates();
        player.visual.SetActiveShield(false);
    ...
    {
        if (curJumpCount >= stat.MaxJumpCount)
            return;
    ...
    private void StartGuard()
    {
        ...
        {
            if (_guardGauge <= 0f) return;
        }
        else
        {
            if (_guardCooldownTimer > 0f) return;
        }
    ...
    {
        player.SetPerfactGuardState(true);
        yield return cachedPerfectGuardWait;
        player.SetPerfactGuardState(false);
        player.SetGuardState(true);
        guardCor = null;
    }
    #endregion
    ...
    private IEnumerator DashCoroutine()
    {
        ...
        while (Time.time < startTime + stat.dashDuration)
        {
            ...
            float dashSpeed = DOVirtual.EasedValue(dashStartSpeed, dashEndSpeed, dashProgress, stat.DashSpeedEase);
    ...
    private Transform FindBestSlamTarget(Vector2 slamDir)
    {
        ...
        if (stat.SlamDetectRange <= 0f)
            return null;

        ContactFilter2D filter = default;
        filter.SetLayerMask(stat.EnemyLayer);
        filter.useTriggers = false;

        int hitCount = Physics2D.OverlapCircle(rb.position, stat.SlamDetectRange, filter, slamTargetResults);
        ...
        {
            Collider2D hit = slamTargetResults[i];
            if (hit == null)
                continue;

            Transform target = hit.attachedRigidbody != null ? hit.attachedRigidbody.transform : hit.transform;
            if (target == null)
                continue;

            if (!IsValidSlamAutoAimTarget(target))
                continue;
            ...
            float angle = Vector2.Angle(slamDir, toTarget.normalized);
            if (angle > stat.SlamAutoAimAngle)
                continue;

            if (angle < bestAngle || (Mathf.Approximately(angle, bestAngle) && sqrDistance < bestSqrDistance))
            {
                bestAngle = angle;
    ...
    private bool IsValidSlamAutoAimTarget(Transform target)
    {
        string[] tags = stat.SlamAutoAimTags;
    ...
    {
        float fallbackDuration = Mathf.Max(MinSlamEaseDuration, stat.SlamEaseFallbackDuration);
        float maxDuration = Mathf.Max(fallbackDuration, fallbackDuration * MaxSlamEaseFallbackMultiplier);

        float maxCheckDistance = Mathf.Max(
            stat.GroundCheckDistance,
            stat.SlamSpeed * Mathf.Max(0.01f, stat.SlamLandingWindow)
        );

        Vector2 origin = groundCheck != null ? (Vector2)groundCheck.position : rb.position;
        RaycastHit2D hit = GetFirstSolidGroundHit(origin, slamDir, maxCheckDistance, stat.GroundLayer);

        if (hit.collider == null)
            return fallbackDuration;

        float startSpeed = stat.SlamSpeed * stat.SlamStartSpeedMultiplier;
        float endSpeed = stat.SlamSpeed * stat.SlamEndSpeedMultiplier;
        float averageSpeed = Mathf.Max(0.01f, (startSpeed + endSpeed) * 0.5f);

        float estimatedDuration = hit.distance / averageSpeed;
        return Mathf.Clamp(estimatedDuration, MinSlamEaseDuration, maxDuration);
    }

    private bool TryGetSlamWallHit(Vector2 slamDir, float distance, out RaycastHit2D wallHit)
    {
        Vector2 origin = groundCheck != null ? (Vector2)groundCheck.position : rb.position;
        int hitCount = Physics2D.RaycastNonAlloc(origin, slamDir, slamBlockHitResults, distance, stat.GroundLayer);
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            if (hit.collider.CompareTag("HalfPlatform"))
                continue;
    ...
    {
        float duration = Mathf.Max(0f, stat.SlamImpactStopDuration);
        if (duration <= 0f)
            yield break;
        ...
        if (_timeManager != null)
            _timeManager.SetActionTime(Mathf.Clamp01(stat.SlamImpactStopTimeScale), duration);
    ...
    private IEnumerator SlamCoroutine()
    {
        ...
        Vector2 slamDir = Mathf.Abs(moveDir.x) >= 0.5f
            ? new Vector2(Mathf.Sign(moveDir.x), -1f).normalized
            : Vector2.down;
        ...
        ApplySlamStartTimeSlow();
        if (_slamStartTimeSlowDuration > 0f)
            yield return new WaitForSecondsRealtime(_slamStartTimeSlowDuration);
        ...
        while (!player.IsGrounded && elapsed < stat.SlamLandingWindow && !slamInterrupted)
        {
            ...
            float slamSpeed = DOVirtual.EasedValue(slamStartSpeed, slamEndSpeed, slamProgress, stat.SlamSpeedEase);
        ...
        yield return CompleteGroundImpactSlam();
    }

    private void CompleteInterruptedSlam()
    {
    ...
    private void CompleteBlockedSlam()
    {
        player.SetPlayingState(true);
    }

    private IEnumerator CompleteGroundImpactSlam()
    {
    ...
    public Player.GuardType OnKnockback(Vector2 knockbackDir, float knockbackForce, float guardKnockbackMultiplier = -1f)
    {
        ...
        if (player.isPerfactGuard)
            guardType = Player.GuardType.PerfectGuard;
        else if (player.isGuard)
            guardType = Player.GuardType.Guard;

        float appliedGuardKnockbackMultiplier = (guardType == Player.GuardType.Guard)
